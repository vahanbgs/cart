...
cargo run -- build                    # download mods into minecraft/mods/, mirror overrides/
...
3. `Build::run_with` also mirrors `<manifest_dir>/overrides/` on top of `<manifest_dir>/minecraft/` — pack-authored configs/resources overwrite whatever's there. Top-level jars in `overrides/mods/` are rejected loudly
...
The game directory is always `<manifest_dir>/minecraft/`; mods are placed in `<manifest_dir>/minecraft/mods/`. A sibling `<manifest_dir>/overrides/` directory, if present, is copied on top of `minecraft/` on every build — top-level jars under `overrides/mods/` are rejected (mods must be declared in `[mods]`).
